extends Node


"""
PRIO:
	1. Separate HUD
	2. MultiPlayer
	3. 
"""


"""
00. call down signal up ! - DONE
01. One scene One thing - DONE
02. Dynamic parameters as doctionary?
03. save templates as file (YAML?)
04. transform logic (like unit movement, projectiles, targeting) to c#
05. LUA & JSON input
06. Function list:
	-	Targeting
		-	fixed
		-	fifs
		-	ground
		-	random
		-	focused
	-	ATKing
		-	Homing ATK
		-	Area ATK
		-	Calculated ATK
	-	Splash
		-	Simple AoE
		-	Range from impact AoE
		-	body blockable from pmpact
		-	Splash targeting
	-	Overtime effects
	-	Item slots
	-	Upgrades
	-	Resource usage
		-	Resource types
			-	HR
			-	Power
			-	Mana
			-	Faith?
	-	modifier items, boosters, upgrades and stat allocation
		-	Store items as YML/LUA+PNG?
		-	Boosters boost one thing significantly
		-	Items modify basic behaviur, for example multitarget, trap placing etc.
		-	Upgrades can be made spending in-game currency or XP
		-	Stats can be redistributed to give extra for something else
			-	for exampe: give X% range, but take away Y% ASPD
09. make every aspekt work with 2D, 2.5D, 3D assets
10. cleaner projectile removal
"""
