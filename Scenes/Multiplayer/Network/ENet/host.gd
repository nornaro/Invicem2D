...
func host() -> void:
	...
	var res:int = multiplayer.peer_connected.connect(_connnected)
	print_debug("_connnected: ",res)
	res = multiplayer.peer_disconnected.connect(_disconnnected)
	print_debug("_disconnnected: ",res)
	set_process(true)
